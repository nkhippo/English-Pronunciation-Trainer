...
**UI 言語:** en / ja / zh / ko / **fil**（タガログ語・Tier 1+3 実装済み）。語義 gloss.fil は **960/3,059語**（Tier 2 進行中）。未マージ語・連結/弱形ルール文は fil 未整備時 **en にフォールバック**（クラッシュなし）。
...
| 前提 | 状態 |
|------|------|
| gloss 品質（多言語UI） | en/ja/zh/ko 実装済み。**fil: 960/3,059語**（batch01–12 マージ済み。残りは en フォールバック） |
...
| 連結句 RP TTS | 未着手（別タスク） |
| gloss.fil（3,059語） | **進行中** 960/3,059（batch01–12）。未マージ語は en フォールバック |
