...
- **意味（gloss）を現在のUI言語で必ず添える** … gloss は en/ja/zh/ko/fil を保持。UI=fil なら `gloss.fil`（**1,280/3,059語**、未マージ時は `gloss.en` にフォールバック）。
...
| Tier 2 | 語義 gloss（3,059 語） | 🔄 **1,280/3,059**（batch01–16 マージ済み） |
...
| ― | gloss.fil（Tier 2） | 🔄 1,280/3,059（`tools/merge_gloss_fil.py`） |
...
| 多言語学習ガイド（6言語） | ✅ フェーズ1 |
| gloss.fil / cs_rule.fil | 🔄 1,280/3,059 gloss.fil。cs_rule.fil は en フォールバック |
| 連結句 RP TTS | ⬜ |
