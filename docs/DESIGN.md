...
| モード | ok | near | bad |
|---|---|---|---|
| Decode | 綴り完全一致 | Levenshtein距離 ≤ 1（入力4文字以上のとき） | それ以外 |
...
> 実装はステップ3（gloss品質）・ステップ4（B/C語彙）の後。A1/A2で先行検証可。
...
| 優先 | 内容 |
|---|---|
| 高 | 欠落必須語の追加：went, gone, made, said, got, children, men, women, feet ＋ 高頻度屈折形（-ed/-ing/複数・不規則過去） |
| 高 | `neighbors`（音素近傍トップK）を全語に事前計算して付与（Mode B用） |
| 高 | `ex`（記号別例語）の再生成元データとして整合維持 |
| 中 | 本物のB/C語彙の追加（Mode Bの主対象）。CEFRバンドで段階配列 |
| 中 | カジュアル表現の追加：wanna, gonna, kinda, gotta, lemme, gimme, dunno, cuz など（IPA・gloss付き） |
| 中 | 薄い記号の補強：ʒ(8語), ɔɪ(29語), ð(42語) を含む語を追加 |
| ― | gloss品質の是正はステップ3（多言語UI点検）で対応 |

---

## 5. 実装順（推奨）

1. **Mode A リファクタ**（音素軸UI・採点据置・localStorage適応・C1撤去/B1B2統合・reveal gloss・記号例語・TTS固定instructions）。
