...
## UI/UX Phase 1（製品 UI 再設計・Track A 延長）

> Phase 1 UI/UX（目的 4 カード等）の実装トラック。既存のローンチ Phase 0–12（「Phase 1: 運用体系」含む）とは別枠。
...
| **1-D** | ドリル本体（`2a`–`2d`、2 PR 分割） | [~] PR1 (`2a`/`2b`) 完了・PR2 未 | #87 / PR TBD |
...
Track A ローンチ後の Google Search Console データ蓄積を元に、以下を実施:

#### SEO 分析ダッシュボード

- Search Console + Vercel Web Analytics + Ahrefs Webmaster Tools のデータを統合するダッシュボード構築
- 週次 / 月次で自動集計するレポート機能
- 主要キーワードの順位変動、CTR、インデックス状況の可視化
- 言語別 SEO 効果測定（`/en/`, `/ja/`, etc.）

#### 任意改善候補 1: sitemap.xml の x-default を `/en/` に変更

**背景**: 現状の sitemap.xml では hreflang="x-default" が `/` を指しており、Google 推奨に沿った「x-default = 自動振り分けエントリ」設計（F2 案 E2 で決定）。ただし GSC で `/` の「リダイレクトエラー」表示が発生している。

**改善内容**: sitemap.xml の x-default を `/en/` に変更することで、GSC の「リダイレクトエラー」表示が解消される。

**判断タイミング**: ローンチ後 1-2 ヶ月の GSC データ蓄積後、`/` のインデックス登録が本当に必要かを評価。多くの場合、各言語別 URL がインデックスされていれば十分。

**影響ファイル**: `sitemap.xml`（1 行変更）

#### 任意改善候補 2: Bot 向け middleware.ts を 302 → rewrite に変更

**背景**: 現状の middleware.ts は Bot UA 検知時に `/en/` に 302 redirect（F2 PR #40 で Cursor 自己判断による選定）。Vercel Hobby 静的サイトでの動作確実性を優先。

**改善内容**: `@vercel/functions` の `rewrite()` を使い、Bot に対して `/en/` の内容を `/` の URL のまま直接配信。これにより、`/` が独立してインデックス登録される可能性。

**判断タイミング**: ローンチ後 1-2 ヶ月の GSC データを元に、`/` のインデックス登録が SEO 効果に必要かを評価。

**影響ファイル**: `middleware.ts`（数行変更）、`package.json`（`@vercel/functions` 依存追加）

#### 実施フロー

1. ローンチ後 4-6 週間の GSC データ蓄積を待つ
2. `/` のインデックス登録状況、`/en/` の順位、多言語検索の効果を評価
3. 上記 2 つの改善候補の実施是非を判断
4. 実施する場合、それぞれ独立した Issue を起票

### GSC 定常観察の Track B 移行時ルール

Track B 開始時、Search Console データが 3-6 ヶ月分蓄積されているはず。以下のトリガーで SEO 改善 Issue を起票:

- 主要キーワードの順位が Top 20 に入ったら、Featured Snippet 対策 Issue
- 累積被リンクが 10+ になったら、被リンク元コンテンツ分析
- 特定言語版の CTR が他言語比で 50% 以下なら、meta 最適化 Issue
