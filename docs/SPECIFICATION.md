...
- UI 言語: **en / ja / zh / ko / fil**（`localStorage.app_lang`、**152 キー**）
...
UI i18n とは独立。各言語キー（`en`, `ja`, `ko`, `zh-Hans`, `zh-Hant`, `fil`）に8セクション（`welcome` … `how_to_use`）。`welcome` は4段落のナラティブ（アプリのアイデンティティ・化石化のコスト・IPA複利・現実的な学習手段）。モーダルで閲覧。
