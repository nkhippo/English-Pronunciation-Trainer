...
topbar の `#vocabBtn` から起動。プレイ中も利用可。Words（wordlist 4,828語）/ Phrases（connected_speech 201句）タブ。弱形 36 語は Phrases タブに含まない。Words は A→Z ソート・検索（debounce 120ms）・A–Z ジャンプ。各行: 単語 / GA+RP IPA / 意味（`vocabDisplayGloss`）/ 品詞 / TTS。RP 行は常に表示。`ipa === rp_ipa` のとき RP 行に `/ipa/（同じ）` 形式。英語 UI では `gloss.en === w` の自己参照を `def` または `(品詞)` で代替。Phrases は cs_type × level 順、タイプバッジ付き。モバイル（`max-width: 599px`）では検索欄を非表示。Escape で閉じる。i18n: `vocab.*`（5キー × 6言語）。
...
約 **4,828 語**（オリジナル 3,059 + Phase 1 M1–M5 で +1,769 B1）。主要フィールド:
...
- 値: `"A1"` / `"A2"` / `"B1"` / `"B2"`
- 分布: A1=1,187 / A2=1,195 / **B1=2,116** / B2=330
