...
将来（RP neighbors 再計算時）:
- 距離入力を `rp_ipa` に切替、RP多重記号を MULTI に追加
- 出力フィールドを `neighbors_rp` とし GA `neighbors` は保持
- 判断・トリガー: docs/rp-neighbors-priority-decision.md
- 監査: scripts/audit_rp_neighbors.py
"""
import json, sys
